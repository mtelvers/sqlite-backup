...
let sqlite_backup t_src t_dst =
  ...
  let () =
    List.iter
      (fun sql -> exec src sql |> or_fail sql)
  ...
  let journal_mode =
    ...
    | err -> Fmt.failwith "Sqlite3 page_size error: %s" (Rc.to_string err)
  in
  let page_size =
  ...
  let () =
    List.iter
      (fun sql -> exec dst sql |> or_fail sql)
      [
  ...
  let () =
    Fmt.pr "Backing up %i pages from %s\n" (Backup.remaining backup) t_src
  in
  ...
  let () =
    Fmt.pr "Backup up %i pages to %s in %f seconds\n" (Backup.pagecount backup)
      t_dst (stop -. start)
  in
  let () = Backup.finish backup |> or_fail "backup finished" in
  let () =
    exec dst ("PRAGMA journal_mode=" ^ journal_mode)
    |> or_fail "failed to set journal_mode"
  in
  let () =
    if not (db_close src && db_close dst) then
...
let cmd =
  ...
  let man =
    [ `S Manpage.s_bugs; `P "Email bug reports to <mark@tarides.com>." ]
  in
